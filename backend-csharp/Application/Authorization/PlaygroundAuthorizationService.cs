...
public class PlaygroundAuthorizationService : IPlaygroundAuthorizationService
{
    ...
    private async Task<PlaygroundMember> GetMembershipAsync(
        Guid playgroundId,
        Guid userId)
    {
        return await _memberRepository
            ...
            ?? throw new UnauthorizedException(
                "You are not a member of this playground.");
    }

    public async Task EnsureCanViewPlaygroundAsync(
        Guid playgroundId,
        Guid userId)
    {
    ...
    public async Task EnsureCanCreateTransactionAsync(
        Guid playgroundId,
        Guid userId)
    {
    ...
    public async Task EnsureCanApproveTransactionAsync(
        Guid playgroundId,
        Guid userId)
    {
        var member = await GetMembershipAsync(playgroundId, userId);
    ...
    public async Task EnsureCanInviteUsersAsync(
        Guid playgroundId,
        Guid userId)
    {
        var member = await GetMembershipAsync(playgroundId, userId);
    ...
    public async Task EnsureCanManagePlaygroundAsync(
        Guid playgroundId,
        Guid userId)
    {
        var member = await GetMembershipAsync(playgroundId, userId);
        ...
        {
            throw new UnauthorizedException(
                "Only the owner can manage this playground.");
        }
    }
}
